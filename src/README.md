...
| `adapters/` | the ways macOS is reached — every protected access goes through the Swift helper (ADR-0002), which `adapters/native/` alone launches — and, outside `native/`, where one context answers another's port, such as the contact names Messages asks for | the ports it implements, and domain types |
