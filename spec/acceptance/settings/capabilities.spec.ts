...
import { listEventsTool } from "../../../src/mcp/calendar/list-events-tool.js";
import { serverOffering } from "../../../src/mcp/server.js";
import {
  aToolThatCreatesAnEvent,
  aToolThatNamesNoCapability,
} from "../../support/a-tool-that-creates-an-event.js";
...
// switched on. `create_event` is stood in for until #36 builds it: the rule belongs to the
// server, not to any one tool, and every write tool that arrives is held to it.

/** A client of a server whose settings came from this client configuration. */
const connectedWith = async (
  configuration: Record<string, string>,
): Promise<{ client: Client; created: unknown[] }> => {
  const settings = settingsFrom(configuration);
  const { tool, created } = aToolThatCreatesAnEvent();
  const listEvents = listEventsTool({
  ...
  return { client: await connectedTo(serverOffering([listEvents, tool], settings)), created };
};
...
describe("switching capabilities on", () => {
  ...
  it("given a client's configuration that names no capability, offers only the tools that read: a write is off until it is named", async () => {
    ...
    expect(await offered(client)).toStrictEqual(["list_events"]);
  });

  it("given a write capability the client's configuration names, offers its tool and carries out a call", async () => {
    const { client, created } = await connectedWith({
      APPLE_NATIVE_MCP_CAPABILITIES: "create_event",
    });

    const result = await client.callTool({ name: "create_event", arguments: { title: "Lunch" } });

    expect(await offered(client)).toStrictEqual(["list_events", "create_event"]);
    expect(result.structuredContent).toStrictEqual({ title: "Lunch" });
    expect(created).toStrictEqual([{ title: "Lunch" }]);
  });
